...
namespace MangaAlert.Scheduler
{
  public class AlertScrapperJob: IHostedService, IDisposable
  {
    ...
    {
      var doc = Web.Load(url);
      var nodes = doc.DocumentNode.Descendants("title").First();
    ...
    {
      var doc = Web.Load(url);
      var nodes = doc.DocumentNode.SelectSingleNode("//ol[@class='list-ep']/li[@class='normal_ep '][last()]/a/div[2]");
    ...
    private async void DoWork(object state)
    {
      ...
      foreach (var url in allUniqueUrls) {
        try {
          ...
          var releaseSchedule = await _trackerRepository.GetReleaseScheduleForUrl(url);

          int latestRelease;
          var domainNameOfUrl = new Uri(url).Host;

          if (releaseSchedule.Any() && releaseSchedule.First() != null) {
            if ((DayOfWeek)Enum.Parse(typeof(DayOfWeek), releaseSchedule.First()) != DateTime.Now.DayOfWeek) {
              return;
            }
          }
